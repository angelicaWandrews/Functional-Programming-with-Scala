...
object Tree {
  ...
  */
  ...
  //  returns a list (i.e. standard library List) with all elements from the leaves in tree t.
  //  Use the Tree.fold method given from the textbook. Hint: the List.++ method appends two lists
  def toList[A](t: Tree[A]) : List[A] =
    fold(t)(a=>List(a))(_++_)
}
object p3{
  def main(args: Array[String]): Unit={
    val treeOne:Tree[Int] =Branch(Branch(Branch(Leaf(1),Leaf(2)),Branch(Leaf(4),Leaf(5))),Branch(Branch(Leaf(1),Leaf(2)),Branch(Leaf(4),Leaf(5))))
    println(Tree.fold(treeOne)(a=>a)(_*_))

    val tree: Tree[Int] = Branch(Branch(Leaf(2), Leaf(3)), Branch(Leaf(4), Leaf(5)))
    println(Tree.map(tree)("leaf "+_.toString()))
    val treeTwo: Tree[String] = Branch(Branch(Leaf("leaf 2"),Leaf("leaf 3")),Branch(Leaf("leaf 4"),Leaf("leaf 5")))
    println(treeTwo)
    println(Tree.toList(treeTwo))   // prints List(leaf 2, leaf 3, leaf 4, leaf 5)
  }
}
